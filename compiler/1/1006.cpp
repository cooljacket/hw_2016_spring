...
int main() {
	...
	bool start, end;
	vector<int> Q0, F;
	vector<T> transaction_input;
	map<int, vector<T> > node_to_t;
	MAPS NDFA;
	...
	for (int i = 0; i < n; ++i) {
		cin >> from >> to >> ch;
		T transaction = (T) {from, to, ch};
		transaction_input.push_back(transaction);
		node_to_t[from].push_back(transaction);
		NDFA[make_pair(from, ch)].push_back(to);
	}
	...
	while (true) {
		...
		for (int i = 0; i < transaction_input.size(); ++i) {
			from = transaction_input[i].from, to = transaction_input[i].to;
			ch = transaction_input[i].x;
			if (ch == Empty) {
				haveEmpty = true;
				map<int, vector<T> >::iterator it = node_to_t.find(to);
				if (it != node_to_t.end()) {
					vector<T>& trs = it->second;
					for (int j = 0; j < trs.size(); ++j) {
						NDFA[make_pair(from, trs[j].x)].push_back(trs[j].to);
						node_to_t[from].push_back((T) {from, trs[j].to, trs[j].x});
						transaction_input.push_back(trs[j]);
					}
				}
				// transaction_input.erase(transaction_input.begin() + i);
				// NDFA.erase(make_pair(from, ch));
				// node_to_t.erase(it);
				break;
			}
		}
